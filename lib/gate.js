...
class GateOracle {
            ...
            if (pool) {
                pool.getConnection((err, connection) => {
                    ...
                    if (err) {
                        ...
                        strSql = "DELETE GATES WHERE ID = :1";
                        connection.execute(strSql, [id], {autoCommit: true}, (err, data) => {
                            this.cn.doRelease(connection);
                            if (err) {
                                reject({
                                    status: "ERROR",
    ...
    getCamionesDisponibles(destino) {
        return this.getGatesInOrOut({ destino: destino, terminal: "ZAP" }, { skip: 0, limit: 10000 });
    }
                    ...
                    if (err) {
                        ...
                        strSql = `SELECT ID,
                                ...
                                FROM GATES
                                    ...
                                    ${strWhere} `;
                        let data = [];
                        let stream = connection.queryStream(strSql);
                        stream.on("error", err => {
                            reject({
                                status: "ERROR",
                                message: err.message,
                        ...
                        stream.on("metadata", () => {
                            data.push("TERMINAL|CONTENEDOR|BUQUE|VIAJE|FECHA|ENTRADA|SALIDA|DURACION|TURNOINICIO|TURNOFIN|CARGA|TIPO|MOV|CAMION|TREN|LARGO");
                        });
                        stream.on("data", item => {
                            var momentIn = require("../include/moment.js");
                            data.push(
                                `${item.TERMINAL}|${item.CONTENEDOR}|${item.BUQUE}|${item.VIAJE}|
                                ${item.GATETIMESTAMP}|${item.GATETIMESTAMP_IN}|${item.GATETIMESTAMP_OUT}|
                                ${momentIn.durationPeriod(item.GATETIMESTAMP_IN, item.GATETIMESTAMP_OUT)}|,
                                ${item.TURNOINICIO}|${item.TURNOFIN}|${item.CARGA}|${item.TIPO}|${item.MOV}|
                                ${item.PATENTECAMION}|${item.TREN}|${item.LARGO}`);
                        });
                        stream.on("end", ()=>{
    ...
    getGatesInOrOut(params, options) {
        return new Promise((resolve, reject) => {
            var strSql,
                strWhere = "";
            var pool = this.cn.pool;
            ...
            if (pool) {
                ...
                if (params.terminal || params.buque || params.viaje || params.contenedor || params.carga || params.tren || params.patenteCamion || params.fechaInicio || params.fechaFin || params.size || params.ontime) {
                    strWhere += " AND ";
                }
                ...
                if (params.destino) {
                    strWhere += ` DESTINO = '${params.destino}' AND `;
                }
                ...
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        strSql = `insert into GATES
                                    ...
                                    :patenteCamion,:patenteAcoplado, :tren, to_date(:registrado_en, 'YYYY-MM-DD HH24:MI:SS'), :largo, :iso, :destino, :dni ) RETURNING ID INTO :outputId`;
                        values = {
                            ...
                            patenteAcoplado: gate.patenteAcoplado,
                            tren: gate.tren,
                        ...
                        connection.execute(strSql, values, { autoCommit: true }, (err, result) => {
                            ...
                            if (err) {
                                ...
                                this.getById(result.outBinds.outputId[0])
                                    .then(data => {
                                        resolve(data);
                                    })
                                    .catch(err => {
            ...
            if (pool) {
                ...
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        connection.execute(strSql, [], {autoCommit: true}, (err, data) => {
                            this.cn.doRelease(connection);
                            if (err) {
                                reject({
                                    status: "ERROR",
        ...
        return new Promise((resolve, reject) => {
            ...
            if (pool) {
                ...
                pool.getConnection((err, connection) => {
                    if (err) {
                        this.cn.doRelease(connection);
                        console.log("%s, Error en Oracle setPedido.", new Date());
                        reject(err);
                    } else {
                        connection.execute(strSql, [], {autoCommit: true}, (err, data) => {
                            this.cn.doRelease(connection);
...
class GateMongoDB {
    ...
    getById(id) {
        return new Promise((resolve, reject) => {
            reject({
                status: "ERROR",
                message: "NOT IMPLEMENTED - getById",
                data: id
            });
        });
    }
    ...
    getCamionesDisponibles(destino) {
        return new Promise((resolve, reject) => {
            reject({
                status: "ERROR",
                message: "NOT IMPLEMENTED - getCamionesDisponibles",
                data: destino
            });
        });
    }
    ...
    getGates(params) {
        return new Promise((resolve, reject) => {
            ...
            taskAsync = asyncCallback => {
                let gates = Gate.find(params, { __v: 0 })
                    .limit(limit)
                    .skip(skip);
                if (order) {
    ...
    getGatesCSV(id) {
        return new Promise((resolve, reject) => {
            reject({
                status: "ERROR",
                message: "NOT IMPLEMENTED - getByIdCSV",
                data: id
            });
        });
    }

    getGatesInOrOut(param, options) {
        return new Promise((resolve, reject) => {
            reject({
                status: "ERROR",
    ...
    getByHour(params) {
        return new Promise((resolve, reject) => {
            var moment = require("moment"),
                jsonParam;
            params.fechaInicio = moment(params.fechaInicio, [
                "YYYY-MM-DD"
            ]).toDate();
            params.fechaFin = moment(params.fechaFin, ["YYYY-MM-DD"])
                .add(1, "days")
                .toDate();
            ...
            jsonParam = [
                ...
                },
                {
                    $project: {
                        gateTimestamp: {
                            $subtract: ["$gateTimestamp", 180 * 60 * 1000]
                        },
                        terminal: "$terminal"
                    }
                },
        ...
        return new Promise((resolve, reject) => {
            ...
            _rates.rates((err, rates) => {
                ...
                taskAsync = asyncCallback => {
                    ...
                    let gates;
                    gates = Gate.find(
                        { terminal: params.terminal, carga: "LL" },
                        { contenedor: true, _id: false }
                    );
        ...
        return new Promise((resolve, reject) => {
            ...
            _rates.rates(function(err, rates) {
                ...
                async.parallel(tasksAsync, () => {
                    ...
                    gatesWoGates = gatesWo
                        .except(invoicesWo, "$.contenedor")
                        .toArray();
    ...
    setStatus(id, status) {
        return new Promise((resolve, reject) => {
            reject({
                status: "ERROR",
                message: "NOT IMPLEMENTED - setStatus",
                data: id
            });
        });
    }
    ...
    setPedido(id) {
        return new Promise((resolve, reject) => {
            reject({
                status: "ERROR",
                message: "NOT IMPLEMENTED - setPedido"
            });
        });
    }
...
class Gate extends Constantes {
    ...
    validate(object) {
        ...
        validate
            .validate("mov", {
                ...
                isIn: "mov must be in \"IMPO\" or \"EXPO\" or \"PASO\" or \"ESTACIONA\", \"CARGAGRAL\" or \"VACIORET\", \"VACIODEV\"values."
            })
            .isLength(1)
            .isIn([
                "EXPO",
        ...
        validate
            ...
            .isDate();
        validate
            .validate("turnoInicio", "turnoInicio must be a valid date", {
                ignoreEmpty: true
            })
            .isDate();
        validate
            .validate("turnoFin", "turnoFin must be a valid date", {
                ignoreEmpty: true
            })
            .isDate();
        validate
            .validate("contenedor", "Contenedor is invalid", {
                ignoreEmpty: true
            })
            .isContainer();
    ...
    getCamionesDisponibles(destino) {
        return this.clase.getCamionesDisponibles(destino);
    }
    ...
    getGatesCSV(params) {
        return this.clase.getGatesCSV(params);
    }

    getGatesInOrOut(params, options) {
        return this.clase.getGatesInOrOut(params, options);
    }
     ...
     *
     * @param {Object} params - Objeto Filtro.
     * @param {String} params.fechaInicio - Fecha inicial de Emision
     * @param {String} params.fechaFin - Fecha final de Emision.
     * @api public
     */
    getByMonth(params) {
        var moment = require("moment");
        ...
        params.fechaFin = moment([fechaFin.year(), fechaFin.month(), 1])
            .add(1, "month")
            .toDate();
    ...
    setStatus(id, status) {
        return this.clase.setStatus(id, status);
    }

    setPedido(id) {
        return this.clase.setPedido(id);
    }
