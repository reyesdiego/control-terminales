...
class InvoiceOracle {
    constructor(oracle) {
        const VoucherType = require("../lib/voucherType");
        this.cn = oracle;
        this.vouchertypes = new VoucherType(oracle);
    }
    ...
    add(invoice, io, callback) {
        ...
        var codTipoComprob = parseInt(
            invoice.codTipoComprob.toString().trim(),
            10
        );
        ...
        if (pool) {
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    console.log(
                        "%s, Error en Oracle Adding Invoice in OracleDb.",
                        new Date()
                    );
                    callback({
                        status: "ERROR",
                        message: "Error en Oracle addInvoice",
                        data: err
                    });
                } else {
                    usr = invoice.usr;
                    ...
                    this.vouchertypes.getById(codTipoComprob)
                        .then(data => {
                            if (data.data === null) {
                                callback({
                                    status: "ERROR",
                                    message: `El Tipo de Comprobante ${codTipoComprob} no existe.`
                                });
                            } else {
                                var voucherType = data.data;
                                ...
                                param = {
                                    outputId: {
                                        type: self.cn.oracledb.NUMBER,
                                        dir: self.cn.oracledb.BIND_OUT
                                    },
                                    ...
                                    fechaDesde: moment(
                                        invoice.fechaServDesde,
                                        "YYYY-MM-DD"
                                    ).format("YYYY-MM-DD"),
                                    fechaHasta: moment(
                                        invoice.fechaServHasta,
                                        "YYYY-MM-DD"
                                    ).format("YYYY-MM-DD"),
                                    fechaVctoPago: moment(
                                        invoice.fechaVctoPago,
                                        "YYYY-MM-DD"
                                    ).format("YYYY-MM-DD"),
                                    resend: invoice.resend,
                                    registrado_en: moment().format(
                                        "YYYY-MM-DD HH:mm:ss"
                                    )
                                };
                                    ...
                                    (err, result) => {
                                        ...
                                        if (err) {
                                                        ...
                                                        if (errResend) {
                                                            ...
                                                            if (
                                                                ...
                                                                WHERE NRO_COMPROB = ${param.nroComprob} AND
                                                                   ...
                                                                   TERMINAL = '${param.terminal}' AND
                                                                   NRO_PTO_VENTA = ${param.nroPtoVenta}`;
                                                                connection.execute(strSql, [], {autoCommit: true},
                                                                    err => {
                                                                        if (err) {
                                                                            self.cn.doRelease(
                                                                                connection
                                            ...
                                            let subTotalCheck = 0,
                                                estado = "Y";
                                            var comment =
                                                "Comprobante transferido correctamente.";
                                            var commentState = "Y";
                                            ...
                                            if (
                                                ...
                                                invoice.detalle.forEach(detalle => {
                                                    ...
                                                    if (
                                                        ...
                                                        detalle.items.forEach(
                                                            item => {
                                                                subTotalCheck += Math.abs(
                                                                    item.impTot
                                                                );
                                                                ...
                                                                task = callbackDetail => {
                                                                    ...
                                                                    let fecha =
                                                                        ...
                                                                        detalle.fecha !==
                                                                            null
                                                                            ? moment(detalle.fecha, "YYYY-MM-DD")
                                                                            : "";
                                                        ...
                                                        let errMsg = util.format(
                                                            "El contenedor no posee items. %s, %j",
                                                            usr.terminal,
                                                            invoice
                                                        );
                                                        if (callback) {
                                                ...
                                                //Verifica que haya Detalle, sino retorna ERROR
                                                let errMsg = util.format(
                                                    "El Comprobante no posee Detalles. %s, %j",
                                                    usr.terminal,
                                                    invoice
                                                );
                                                if (callback) {
                                            ...
                                            if (
                                                ...
                                                comment = `El subtotal del comprobante es incorrecto, la suma es ${subTotalCheck} y se informa ${
                                                    invoice.impSubtot
                                                }. - ${usr.terminal}.`;
                                                estado = "T";
                                            }
                    ...
                    var strVoucher =
                        "SELECT TYPE FROM VOUCHER_TYPE WHERE ID = :1";
                    connection.execute(
                        strVoucher,
                        [codTipoComprob],
                        {},
        ...
        return new Promise((resolve, reject) => {
            ...
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    reject({
                        status: "ERROR",
                        message: "Error en Oracle addState",
                        data: err
                    });
                } else {
                    let strSql;
            ...
            if (pool) {
                taskAsync = asyncCallback => {
                    pool.getConnection((err, connection) => {
                        if (err) {
                            console.log(
                                "%s, Error en Oracle getInvoice.",
                                new Date()
                            );
                            self.cn.doRelease(connection);
                            reject({
                                status: "ERROR",
                                message: "Error en Oracle getInvoice",
                                data: err
                            });
                        } else {
                            strSql = `SELECT ID,
                                        ...
                                        if (data.rows) {
                                            data = data.rows.map(item => ({
                                                ...
                                                importe: {
                                                    ...
                                                    noGravado:
                                                        item.IMPORTE_NO_GRAVADO,
                                                    exento: item.IMPORTE_EXENTO,
                                                    subtotal:
                                                        item.IMPORTE_SUBTOTAL,
                                                    iva: item.IMPORTE_IVA,
                                                    importe_otros_tributos:
                                                        item.IMPORTE_OTROS_TRIBUTOS,
            ...
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    if (params.fechaInicio) {
                        strWhere += util.format(
                            " FECHA_EMISION >= TO_DATE('%s', 'RRRR-MM-DD') AND ",
                            moment(params.fechaInicio, "YYYY-MM-DD").format(
                                "YYYY-MM-DD"
                            )
                        );
                    }
                    if (params.fechaFin) {
                        strWhere += util.format(
                            " FECHA_EMISION <= TO_DATE('%s', 'RRRR-MM-DD') AND ",
                            moment(params.fechaFin, "YYYY-MM-DD").format(
                                "YYYY-MM-DD"
                            )
                        );
                    }
                    ...
                    if (params.codTipoComprob) {
                        strWhere += util.format(
                            " COD_TIPO_COMPROB = %s AND ",
                            params.codTipoComprob
                        );
                    }
                    if (params.nroComprobante) {
                        strWhere += util.format(
                            " NRO_COMPROB = %s AND ",
                            params.nroComprobante
                        );
                    }
                    ...
                    if (params.estado) {
                        strWhere += util.format(
                            " STATE IN (%s) AND ",
                            params.estado.split(",").map(state => `'${state}'`)
                        );
                    }
                    if (params.resend) {
                        strWhere += util.format(
                    ...
                    if (strWhereExists.length > 0) {
                        strWhereExists = util.format(
                            "( SELECT * FROM INVOICE_DETAIL I LEFT JOIN ISO3 ON SUBSTR(I.ISO, 3,1) = ISO3.ID WHERE IH.ID = INVOICE_HEADER_ID AND %s )",
                            strWhereExists.substr(0, strWhereExists.length - 4)
                        );
                        strWhere += util.format(
                            " EXISTS %s AND ",
                            strWhereExists
                    ...
                    if (strWhere.length > 0) {
                        strWhere = util.format(
                            " WHERE %s ",
                            strWhere.substr(0, strWhere.length - 4)
                        );
                    }
                    ...
                    async.parallel(tasksAsync, (err, data) => {
                        ...
                        if (err) {
                            ...
                            if (invoices) {
                                ...
                                if (callback) {
                                    return callback(undefined, invoices);
                                }
                            } else {
                                if (callback) {
                                    return callback({
                                        status: "ERROR",
                                        message: "ERROR en getInvoices"
                                    });
                                }
                            }
                        }
                ...
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        stream.on("metadata", function(metadata) {
                            arr.push(
                                "TIPO|COD_TIPO_COMPROB|NRO_PTO_VENTA|NRO_COMPROB|RAZON|IMPORTE_TOTAL|TOTAL|COD_MONEDA|COTI_MONEDA|FECHA_EMISION|CONTENEDOR|BUQUE_NOMBRE|BUQUE_VIAJE|BUQUE_FECHA|CODE|CNT|IMP_UNIT|IMP_TOT|ISO"
                            );
                        });

                        stream.on("data", data => {
                            arr.push(
                                `${data.TIPO}|${data.COD_TIPO_COMPROB}|${
                                    data.NRO_PTO_VENTA
                                }|${data.NRO_COMPROB}|${data.RAZON}|${
                                    data.IMPORTE_TOTAL
                                }|${data.TOTAL}|${data.COD_MONEDA}|${
                                    data.COTI_MONEDA
                                }|${data.FECHA_EMISION}|${data.CONTENEDOR}|${
                                    data.BUQUE_NOMBRE
                                }|${data.BUQUE_VIAJE}|${data.BUQUE_FECHA}|${
                                    data.CODE
                                }|${data.CNT}|${data.IMP_UNIT}|${
                                    data.IMP_TOT
                                }|${data.ISO}`
                            );
                        });
        ...
        if (pool) {
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    strSql =
                        "SELECT DISTINCT RAZON FROM INVOICE_HEADER WHERE TERMINAL = :1 ORDER BY RAZON";
                    connection.execute(
                        strSql,
                        [params.terminal],
                        { outFormat: self.cn.oracledb.ARRAY, resultSet: true },
                        (err, data) => {
                            if (err) {
                                if (callback) {
                                    return callback({
            ...
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    if (params.fecha !== undefined) {
                        fechaEmision = moment(params.fecha, [
                            "YYYY-MM-DD"
                        ]).format("YYYY-MM-DD");
                    }
                        ...
                        (err, data) => {
                            ...
                            if (err) {
                                ...
                                response = Enumerable.from(data.rows)
                                    .groupBy(
                                        "x=>JSON.stringify({cod: x.COD_TIPO_COMPROB, desc: x.DESCRIPTION})",
                                        item => {
                                            return item;
                                        },
                                        (job, grouping) => {
    ...
    getCashbox(params, callback) {
        ...
        if (pool) {
            ...
            if (params.fechaFin) {
                strWhere += util.format(
                    " FECHA_EMISION <= TO_DATE('%s', 'RRRR-MM-DD') AND ",
                    moment(params.fechaFin, "YYYY-MM-DD").format("YYYY-MM-DD")
                );
            }
            if (params.nroPtoVenta) {
                strWhere += util.format(
            ...
            if (strWhereExists.length > 0) {
                strWhereExists = util.format(
                    "( SELECT * FROM INVOICE_DETAIL WHERE INVOICE_HEADER.ID = INVOICE_HEADER_ID AND %s )",
                    strWhereExists.substr(0, strWhereExists.length - 4)
                );
                strWhere += util.format(" EXISTS %s AND ", strWhereExists);
            }
                    ...
                    connection.execute(strSql, [], {}, (err, data) => {
                        ...
                        if (err) {
                            ...
                            if (data.rows.length > 0) {
                                puntos = data.rows.map(
                                    item => item.NRO_PTO_VENTA
                                );
                            }
        ...
        return new Promise((resolve, reject) => {
            ...
            if (pool) {
                ...
                pool.getConnection((err, connection) => {
                    if (err) {
                        console.log(
                            "%s, Error en Oracle getCorrelative.",
                            new Date()
                        );
                        this.cn.doRelease(connection);
                        reject({
                        ...
                        strSql =
                            "BEGIN sp_correlative_NEW(:P_TERMINAL, :P_COD_TIPO_COMPROB, :P_INICIO, :P_FIN, :RESULT_CURSOR); END;";
                        param = {
                            ...
                            RESULT_CURSOR: {
                                type: this.cn.oracledb.CURSOR,
                                dir: this.cn.oracledb.BIND_OUT
                            }
                        };
            ...
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    connection.execute(
                        strSql,
                        [params.terminal],
                        { outFormat: self.cn.oracledb.ARRAY },
                        (err, data) => {
                            self.cn.doRelease(connection);
                            if (err) {
                                if (callback) {
                ...
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        if (param.fechaInicio) {
                            strWhere += ` FECHA_EMISION >= TO_DATE('${moment(param.fechaInicio, "YYYY-MM-DD").format("YYYY-MM-DD")}', 'RRRR-MM-DD') AND `;
                        }
                        if (param.fechaFin) {
                            strWhere += ` FECHA_EMISION <= TO_DATE('${moment(param.fechaFin, "YYYY-MM-DD").format("YYYY-MM-DD")}', 'RRRR-MM-DD') AND `;
                        }
                        ...
                        const strSql = `SELECT *
                                        FROM (
                                            SELECT I.ID, 
                                                ...
                                                ISO3, 
                                                ISO4,
                                                ROW_NUMBER() OVER (ORDER BY I.ID ) R
                                            FROM V_INVOICE_HEADER_DETAIL I
                                                LEFT JOIN TARIFARIO_TERMINAL TT ON I.CODE = TT.CODE AND
                                                                                    I.TERMINAL = TT.TERMINAL
                                                LEFT JOIN TARIFARIO T ON T.ID = TT.TARIFARIO_ID
                                            ${strWhere}
                                            ORDER BY FECHA_EMISION, I.TERMINAL, NRO_PTO_VENTA, NRO_COMPROB, CONTENEDOR
                                        )
                                        WHERE R BETWEEN ${options.skip} AND ${options.skip + options.limit}`;
                        ...
                        stream.on("data", data => {
                            arr.push({
                                ...
                                nroPtoVenta: data.NRO_PTO_VENTA, 
                                nroComprob: data.NRO_COMPROB, 
                                codTipoComprob: data.COD_TIPO_COMPROB,
                                fecha: data.FECHA_EMISION, 
                                razonSocial: data.RAZON,
                                cotiMoneda: data.COTI_MONEDA,
                                total: data.IMPORTE_TOTAL, 
                                contenedor: data.CONTENEDOR, 
                                buque: data.BUQUE_NOMBRE,
                                viaje: data.BUQUE_VIAJE,
                                iso: data.ISO, 
                                tarifa: data.CODE, 
                                impUnit: data.IMP_UNIT, 
                                cantidad: data.CNT, 
                                impTotal: data.IMP_TOT, 
                                iso1: data.ISO1, 
                                iso2: data.ISO2, 
                                iso3: data.ISO3, 
                                iso4: data.ISO4
            ...
            if (pool) {
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        strSql =
                            "SELECT MAX(REGISTRADO_EN) REGISTRADO_EN FROM INVOICE_HEADER WHERE TERMINAL = :1";
                        connection.execute(
                            strSql,
                            [terminal],
                            {},
    ...
    getRatesByPeriod(params) {
        return new Promise((resolve, reject) => {
            ...
            if (params.fechaInicio) {
                strWhere += ` FECHA_EMISION >= TO_DATE('${moment(
                    params.fechaInicio,
                    "YYYY-MM-DD"
                ).format("YYYY-MM-DD")}', 'RRRR-MM-DD') AND `;
            }
            if (params.fechaFin) {
                strWhere += ` FECHA_EMISION < TO_DATE('${moment(
                    params.fechaFin,
            ...
            if (pool) {
                ...
                pool.getConnection((err, connection) => {
                    if (err) {
                        console.log(
                            "%s, Error en Oracle getRatesDate",
                            new Date()
                        );
                        this.cn.doRelease(connection);
                        reject({
    ...
    getRatesByTerminal(params) {
        return new Promise((resolve, reject) => {
            ...
            if (params.estado) {
                strWhere += ` STATE IN (${params.estado
                    .split(",")
                    .map(state => `'${state}'`)}) AND `;
            }
            if (params.resend) {
            ...
            if (strWhereExists.length > 0) {
                strWhereExists = ` ( SELECT * FROM INVOICE_DETAIL I LEFT JOIN ISO3 ON SUBSTR(I.ISO, 3,1) = ISO3.ID WHERE VH.ID = INVOICE_HEADER_ID AND ${strWhereExists.substr(
                    0,
                    strWhereExists.length - 4
                )} ) `;
            ...
            if (pool) {
                ...
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        connection.execute(
                            strSql,
                            [params.terminal, params.contenedor],
                            {},
                            (err, data) => {
                                if (err) {
                                    console.log(
        ...
        return new Promise((resolve, reject) => {
            ...
            if (pool) {
                pool.getConnection((err, connection) => {
                    if (err) {
                        console.log(
                            "%s, Error en Oracle getNoMatches.",
                            new Date()
                        );
                        reject({
                            status: "ERROR",
                        ...
                        if (params.order) {
                            orderBy = this.cn
                                ...
                                .toUpperCase();
                            var order = orderBy.includes("ASC")
                                ? " ASC"
                                : " DESC";
                            if (orderBy.includes("NROPTOVENTA")) {
                                orderBy = "NRO_PTO_VENTA";
        ...
        if (pool) {
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    if (params.fechaInicio) {
                        strWhere += util.format(
                            " IH2.FECHA_EMISION >= TO_DATE('%s', 'RRRR-MM-DD') AND ",
                            moment(params.fechaInicio, "YYYY-MM-DD").format(
                                "YYYY-MM-DD"
                            )
                        );
            ...
            if (pool) {
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        connection.execute(
                            strSql,
                            [params.terminal, params.buque, params.viaje],
                            { resultSet: true },
                            (err, data) => {
                                if (err) {
                                    reject({
            ...
            if (pool) {
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        if (params.nroPtoVenta) {
                            strWhere += ` NRO_PTO_VENTA = ${
                                params.nroPtoVenta
                            } AND `;
                        }
                        if (params.codTipoComprob) {
                            strWhere += ` COD_TIPO_COMPROB = ${
                                params.codTipoComprob
                            } AND `;
                        }
                        if (params.nroComprobante) {
                            strWhere += ` NRO_COMPROB = ${
                                params.nroComprobante
                            } AND `;
                        }
                        if (params.razonSocial) {
                        ...
                        if (strWhereExists.length > 0) {
                            strWhereExists = `( SELECT * FROM INVOICE_DETAIL I LEFT JOIN ISO3 ON SUBSTR(I.ISO, 3,1) = ISO3.ID WHERE HD.ID = INVOICE_HEADER_ID AND ${strWhereExists.substr(
                                0,
                                strWhereExists.length - 4
                            )} )`;
                            strWhere += ` EXISTS ${strWhereExists} AND `;
                        ...
                        if (strWhere.length > 0) {
                            strWhere = ` WHERE COD_MONEDA = 'DOL' AND ${strWhere.substr(
                                0,
                                strWhere.length - 4
                            )} `;
                        }
                            ...
                            (err, data) => {
                                ...
                                if (err) {
                                    ...
                                    if (options.output === "csv") {
                                        let response =
                                            "CUIT|RAZON|COMP|MINIMA|PROMEDIO|MAXIMA|TOTAL\n";
                                                    ...
                                                    (err, data) => {
                                                        if (err) {
                                                            ...
                                                            if (
                                                                ...
                                                                let response =
                                                                    "TERMINAL|CONTENEDOR|BUQUE|VIAJE|TOTAL|ISO1\n";
                                                                ...
                                                                if (
                                                                    ...
                                                                    let dataResult = Enumerable.from(
                                                                        data.rows
                                                                    )
                                                                        .select(
                                                                            x => ({
    ...
    getInvoicesByRatesTerminal(params, options) {
        return new Promise((resolve, reject) => {
            ...
            if (params.fechaInicio && params.fechaFin) {
                andWhere += ` FECHA_EMISION >= TO_DATE('${
                    params.fechaInicio
                }', 'YYYY-MM-DD') AND `;
                andWhere += ` FECHA_EMISION <= TO_DATE('${
                    params.fechaFin
                ...
                if (params.year) {
                    andWhere += ` TO_CHAR(FECHA_EMISION, 'YYYY') = ${
                        params.year
                    } AND `;
                }
                if (params.month) {
                    andWhere += ` TO_CHAR(FECHA_EMISION, 'MM') = ${
                        params.month
                    } AND `;
                }
            }
            ...
            if (pool) {
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        connection.execute(
                            strSql,
                            [params.container, params.terminal],
                            {},
                            (err, data) => {
                                this.cn.doRelease(connection);
                                if (err) {
...
class InvoiceMongoDB {
    ...
    add(param, io, callback) {
        ...
        try {
            ...
            invoice = {
                ...
                codTipoComprob: parseInt(
                    postData.codTipoComprob.toString().trim(),
                    10
                ),
                ...
                fecha: {
                    ...
                    desde: moment(
                        postData.fechaServDesde,
                        "YYYY-MM-DDT00:00:00.000.Z"
                    ),
                    hasta: moment(
                        postData.fechaServHasta,
                        "YYYY-MM-DDT00:00:00.000.Z"
                    ),
                    vctoPago: moment(
                        postData.fechaVctoPago,
                        "YYYY-MM-DDT00:00:00.000.Z"
                    )
                },
                detalle: [],
            ...
            if (postData.otrosTributos) {
                postData.otrosTributos.forEach(function(item) {
                    var otId =
                        item.id !== undefined ? item.id.toString() : null;
            ...
            if (postData.detalle && postData.detalle.length > 0) {
                postData.detalle.forEach(function(container) {
                    buqueId =
                        container.buqueId !== undefined &&
                        container.buqueId !== null
                            ? container.buqueId.toString()
                            : "";
                    buqueDesc = container.buqueDesc;
                    viaje = container.viaje;
                    fecha =
                        container.fecha !== undefined &&
                        container.fecha !== "" &&
                        container.fecha !== null
                            ? moment(container.fecha, "YYYY-MM-DD")
                            : "";
                    buque = {
            ...
            body = util.format(
                "Error al insertar comprobante. %s. \n%s",
                error.message,
        ...
        invoice2add.save(function(errSave, data) {
            ...
            if (!errSave) {
                ...
                if (
                    subTotalCheck > postData.impSubtot + 2 ||
                    subTotalCheck < postData.impSubtot - 2
                ) {
                    comment = util.format(
                        "El subtotal del comprobante es incorrecto, la suma es %d y se informa %d. - %s.",
                        subTotalCheck,
                        postData.impSubtot,
                        usr.terminal
                    );
                    ...
                    Invoice.find(
                        ...
                        function(err, invoices) {
                            ...
                            if (estado === 1) {
                                Invoice.remove(
                                    { _id: invoices[0]._id },
                                    function(err, delInvoice) {
                                        this.comments.remove(
                                            { invoice: invoices[0]._id },
                                            function(errComment, delComment) {
                                                param.usr = usr;
                                ...
                                errMsg = util.format(
                                    "Error INS: El tipo de comprobante: %s, número: %s, fue transferido el %s:\n - ERROR:%s",
                                    invoices[0].codTipoComprob,
                                    invoices[0].nroComprob,
                                    dateTime.getDateTimeFromObjectId(
                                        invoices[0]._id
                                    ),
                                    errSave
                                );
    ...
    addState(params) {
        return new Promise((resolve, reject) => {
            Invoice.update(
                { _id: params.invoiceId, "estado.grupo": params.group },
                { $set: { "estado.$.estado": params.estado } },
                function(err, rowAffected, data) {
                    if (err) {
                        reject({
                            status: "ERROR",
                            message: "Error en cambio de estado.",
                            data: err
                        });
                    } else {
                        if (rowAffected === 0) {
            ...
            invoice.exec((err, invoices) => {
                ...
                if (err) {
                    ...
                    if (invoices.length > 0) {
                        ...
                        invoice.registrado_en = moment(
                            invoices[0]._id.getTimestamp()
            ...
            param.$or = [
                {
                    estado: {
                        $size: 1,
                        $elemMatch: { estado: { $in: states }, grupo: "ALL" }
                    }
                },
                {
                    "estado.1": { $exists: true },
                    estado: {
                        $elemMatch: {
                            estado: { $in: states },
                            grupo: filtro.group
    ...
    getCounts(params, callback) {
        ...
        tomorrow = moment(fechaEmision)
            .add(1, "days")
            .toDate();
        match.$match = {
            "fecha.emision": { $gte: fechaEmision, $lt: tomorrow }
        };
    ...
    getCountByDate(params, callback) {
        var Invoice = this.model;
        var moment = require("moment");
        var date, date5Ago, tomorrow, jsonParam, result;
        ...
        jsonParam = [
            { $match: { "fecha.emision": { $gte: date5Ago, $lt: tomorrow } } },
            {
                $project: {
                    fecha: "$fecha.emision",
                    accessDate: {
                        $subtract: ["$fecha.emision", 180 * 60 * 1000]
                    },
                    terminal: "$terminal",
                    total: "$total"
                }
    ...
    getCountByMonth(params, callback) {
        ...
        if (params.fecha !== undefined) {
            date = moment(params.fecha, ["YYYY-MM-DD"]).subtract(
                moment(params.fecha, "YYYY-MM-DD").date() - 1,
                "days"
            );
        }
        month5Ago = moment(date)
            .subtract(4, "months")
            .toDate();
        ...
        Invoice.aggregate(jsonParam, function(err, data) {
            if (err) {
                ...
                if (callback) {
                    return callback(undefined, { status: "OK", data: data });
                }
            }
        });
    }
    ...
    getHeaderDetail(param) {
        return new Promise((resolve, reject) => {
            reject({
                status: "ERROR",
                message: "NOT IMPLEMENTED. getHeaderDetil"
            });
        });
    }
            ...
            this.model
                ...
                .exec((err, data) => {
                    if (err) {
                        ...
                        let moment = require("moment");
                        let result = data[0];
                        let fecha = moment(result._id.getTimestamp()).add(
                            lastHours,
                            "hour"
                        );
                        if (fecha < moment()) {
    ...
    getRatesByPeriod(params) {
        return new Promise((resolve, reject) => {
            var desde, hasta, cond, _price, _rates;
            var Enumerable = require("linq");
            var moment = require("moment");
                            ...
                            invoice.exec((err, data) => {
                                ...
                                if (err) {
                                    ...
                                    result = Enumerable.from(data)
                                        .join(
                                            Enumerable.from(prices),
                                            "$.code",
                                            "$.code",
                                            (tasaInvoice, price) => {
                                                tasaInvoice.impUnitAgp =
                                                ...
                                                tasaInvoice.tasaAgp =
                                                    tasaInvoice.impUnitAgp *
                                                    tasaInvoice.cnt;
                                                tasaInvoice.totalTasa =
                                                    tasaInvoice.tasa *
                                                    tasaInvoice.cotiMoneda;
                                                tasaInvoice.totalTasaAgp =
                                                    tasaInvoice.tasaAgp *
                                                    tasaInvoice.cotiMoneda;
                                                return tasaInvoice;
                                            }
                                        ...
                                        .groupBy(
                                            "x=>JSON.stringify({code: x.code, terminal: x.terminal, date: x.date})",
                                            null,
                                            (key, g) => {
                                                var r;
                                                key = JSON.parse(key);
                                    ...
                                    mp = this.matchPrice.find(
                                        { match: { $in: rates } },
                                        { price: true, match: true }
                                    );
                                    mp.populate({
                                        path: "price",
                            ...
                            invoice.exec((err, data) => {
                                ...
                                if (err) {
                                    ...
                                    result = Enumerable.from(data)
                                        ...
                                        .groupBy(
                                            "x=>JSON.stringify({code: x.code, terminal: x.terminal})",
                                            null,
                                            (key, g) => {
                                                var r;
                                                key = JSON.parse(key);
        ...
        _rates.rates(function(err, rates) {
            if (rates.length > 0) {
                ...
                if (params.fechaInicio || params.fechaFin) {
                    ...
                    if (params.fechaInicio) {
                        fecha = moment(
                            moment(params.fechaInicio, "YYYY-MM-DD")
                        ).toDate();
                        param["fecha.emision"].$gte = fecha;
                    }
                    if (params.fechaFin) {
                        fecha = moment(
                            moment(params.fechaFin, "YYYY-MM-DD")
                        ).toDate();
                        param["fecha.emision"].$lte = fecha;
                    }
                }
                ...
                invoices.exec(function(err, invoices) {
                    ...
                    Invoice.count(param, function(err, cnt) {
                        ...
                        if (callback) {
                            return callback(undefined, dataResult);
                        }
                    });
                });
            } else {
            ...
            task = function(asyncCallback) {
                ...
                Invoice.aggregate([
                    ...
                    if (err) {
                        ...
                        let contDist = Enumerable.from(data2).select(
                            "$.contenedor"
                        );
                ...
                if (err) {
                    ...
                    rates.rates((err, ratesArray) => {
                        query = [
                            ...
                            {
                                $match: {
                                    "detalle.items.id": { $in: ratesArray }
                                }
                            },
                            {
                                $group: {
        ...
        return new Promise((resolve, reject) => {
            ...
            invoices.exec((err, data) => {
                ...
                if (err) {
                    ...
                    total = Enumerable.from(data).sum("$.total");
...
class Invoice extends Constantes {
    ...
    getHeaderDetail(param, options) {
        return this.clase.getHeaderDetail(param, options);
    }
