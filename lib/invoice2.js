            ...
            if (pool) {
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        strSql = `SELECT *
                                FROM (
                                      SELECT CONTENEDOR, BUQUE_NOMBRE, BUQUE_VIAJE, ISO1, SUM(IMP_TOT) TOTAL_CON, ROW_NUMBER() OVER (ORDER BY  ${strCampo} ${strOrder}  ) R
                                      FROM V_INVOICE_HEADER_DETAIL VHD
                                              INNER JOIN VOUCHER_TYPE V ON V.ID = VHD.COD_TIPO_COMPROB
                                      WHERE TERMINAL = :1 AND LENGTH(CONTENEDOR) = 11 ${strWhere}
                ...
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        strSql = `SELECT IH.TERMINAL, FECHA_EMISION, IH.ID, COD_TIPO_COMPROB, CONTES, NRO_COMPROB, COTI_MONEDA, COD_MONEDA, NRO_PTO_VENTA, RAZON,
                                        ...
                                        BUQUE_NOMBRE, BUQUE_VIAJE, TOTAL, TOTAL_CON, CONTENEDOR, ISO
                                    FROM INVOICE_HEADER IH
                                        INNER JOIN INVOICE_STATE STA ON  IH.ID = STA.INVOICE_HEADER_ID AND
                                                                        STA.ID = (SELECT MAX(ST1.ID) FROM INVOICE_STATE ST1 WHERE ST1.INVOICE_HEADER_ID = IH.ID)
                                        INNER JOIN (SELECT INVOICE_HEADER_ID, COUNT(DISTINCT CONTENEDOR) CONTES FROM INVOICE_DETAIL GROUP BY INVOICE_HEADER_ID) CONTE
