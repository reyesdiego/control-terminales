...
function getResultSet (connection, resultSet, numRows, callbackResultSet) {
    var ret = [];
                                            ...
                                            if (errCommit) {
                                                ...
                                                strSql = `DECLARE R VARCHAR2(20);
                                                            BEGIN
                                                              SELECT RATE
                                                              INTO R
                                                              FROM V_INVOICE_HEADER_DETAIL V
                                                                  INNER JOIN TARIFARIO_TERMINAL TT ON V.TERMINAL = TT.TERMINAL AND
                                                                                                      V.CODE = TT.CODE
                                                                  INNER JOIN TARIFARIO T ON TT.TARIFARIO_ID = T.ID AND
                                                                                                      RATE IS NOT NULL
                                                              WHERE V.ID = :1
                                                              GROUP BY RATE;
...
//                    strSql = `BEGIN sp_correlative_NEW(:P_TERMINAL, :P_NRO_PTO_VENTA, :P_COD_TIPO_COMPROB, :P_INICIO, :P_FIN, :RESULT_CURSOR); END;`;
                    ...
                    connection.execute(strSql, param, {}, (err, data) => {
                        if (data !== undefined) {
                            ...
                            getResultSet(connection, rSet, 1000, (err, data, connect) => {
                                if (err) {
                                    rSet.close(err => {
                                        console.error(err);
                                        this.cn.doRelease(connection);
    ...
    getRatesByTerminal (params) {
        return new Promise((resolve, reject) => {
            ...
            if (pool) {
                ...
                if (params.tasaAgp === '1') {
                    strSql = `SELECT VHD.TERMINAL, ${fecha_emision} FECHA_EMISION, T.CODE, DESCRIPCION,
                                  SUM(VHD.IMP_UNIT*CNT*V.TYPE) TASA,
                                  SUM(PRECIO*CNT*V.TYPE) TASA_AGP,
                            ...
                            WHERE TP.TARIFARIO_ID = T.ID AND
                                  TP.ID = (SELECT MAX(ID) FROM TARIFARIO_PRECIO TP1 WHERE FECHA <= FECHA_EMISION AND TP.TARIFARIO_ID = TP1.TARIFARIO_ID ) AND
                                  RATE IS NOT NULL ${strWhere}
                            GROUP BY VHD.TERMINAL, ${fecha_emision}, T.CODE, DESCRIPCION
                            ORDER BY FECHA_EMISION`;
                } else {
                                ...
                                ["IMPO", "EXPO", "REMOVIDO"].forEach(tipoMov => {
                                    isos.rows.forEach( item => {
                                        tasks.push(asyncCallback => {
                                            ...
                                            strSql = `SELECT *
                                            FROM (
                                                  SELECT CONTENEDOR, BUQUE_NOMBRE, BUQUE_VIAJE, ISO1, SUM(IMP_TOT*v.type) TOTAL_CON, ROW_NUMBER() OVER (ORDER BY  ${strCampo} ${strOrder}  ) R
                                                  FROM V_INVOICE_HEADER_DETAIL VHD
                                                          INNER JOIN VOUCHER_TYPE V ON V.ID = VHD.COD_TIPO_COMPROB
                                                  WHERE TERMINAL = :1 AND TIPO = :2 AND
                                                        ISO1 = '${item.ID}' ${strWhere}
                                                  GROUP BY CONTENEDOR, BUQUE_NOMBRE, BUQUE_VIAJE, ISO1
                                            )
                                            ${strWhereTop}`;
                                            connection.execute(strSql, [params.terminal, tipoMov], {}, (err, data) => {
                                                if (err) {
                                                    asyncCallback(err);
                                                } else {
                                                    if (options.output === 'csv') {
                                                        let response = "CONTENEDOR|BUQUE|VIAJE|TOTAL|ISO1\n";
            ...
            if (pool) {
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        strSql = `SELECT IH.TERMINAL, FECHA_EMISION, IH.ID, COD_TIPO_COMPROB, CONTES, NRO_COMPROB, COTI_MONEDA, COD_MONEDA, NRO_PTO_VENTA, RAZON,
