            ...
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    taskAsync = asyncCallback => {
                        strSql ="SELECT IH.*, USR,  GRUPO,  STATE " +
                                "FROM " +
                                "(SELECT " +
                                "   ID, " +
                                "   TERMINAL, " +
                                ...
                                "FROM INVOICE_HEADER %s) IH " +
                                "   INNER JOIN INVOICE_STATE STA ON " +
                                "        IH.ID = STA.INVOICE_HEADER_ID AND " +
                                "       STA.ID = (SELECT MAX(ID) FROM INVOICE_STATE ST1 WHERE ST1.INVOICE_HEADER_ID = IH.ID) " +
