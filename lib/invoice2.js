                            ...
                            connection.execute(strSql, param, {autoCommit: _autoCommit}, (err, result) => {
                                ...
                                if (err) {
                                    ...
                                    if (invoice.detalle && invoice.detalle.length > 0) {
                                        ...
                                        invoice.detalle.forEach(function (detalle) {
                                            ...
                                            if (detalle.items && detalle.items.length>0) {
                                                ...
                                                //Verifica que haya Items en el Detalle, sino retorna ERROR
                                                let errMsg = util.format("Invoice ORA INS: %s, %j", "El contenedor no posee items.", usr.terminal, postData);
                                        ...
                                        //Verifica que haya Detalle, sino retorna ERROR
                                        let errMsg = util.format("Invoice ORA INS: %s, %j", "El Comprobante no posee Detalles.", usr.terminal, invoice);
                                        if (callback) return callback({
                                            status: "ERROR",
                                            message: errMsg,
                                            data: invoice
                                    ...
                                    taskComment = function (callbackComment) {
                                        ...
                                        connection.execute(strSql, param, {autoCommit: _autoCommit}, (err, resultComment) => {
                                            if (err) {
                                    ...
                                    task = function (callbackState) {
                                        ...
                                        connection.execute(strSql, param, {autoCommit: _autoCommit}, (err, resultState) => {
                                            if (err) {
...
//            tipoResultado = oracle.oracledb.ARRAY;
    ...
    getCorrelative (params, callback ) {
        ...
        if (params.fechaInicio === undefined) {
            params.fechaInicio = moment("2013-01-01", 'YYYY-MM-DD').toDate();
        }
            ...
            if (postData.detalle && postData.detalle.length > 0) {
                postData.detalle.forEach(function (container) {
                    ...
                    if (container.items) {
                        ...
                        errMsg = util.format("Invoice INS: %s, %j", "El contenedor no posee items.", usr.terminal, postData);
                        if (callback) return callback({
                                                status: "ERROR",
                                                message: errMsg,
                                                data: postData
    ...
    getInvoice (params, callback) {
        ...
        invoice.exec(function(err, invoices) {
            ...
            if (err) {
                ...
                if (callback) return callback(undefined, {status:"OK", data: invoice||null});
            }
        });
    }
