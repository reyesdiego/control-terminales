            ...
            if (pool) {
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        strSql = util.format(strSql, strWhere);
                        connection.execute(strSql, [], {resultSet: true}, (err, data) => {
                            if (err) {
                                reject(err);
                            } else {
                                ...
                                getResultSet(connection, data.resultSet, 500)
                                    .then(data => {
                                        ...
                                        if (data) {
                                            var response = "TERMINAL|TIPO|COD_TIPO_COMPROB|NRO_PTO_VENTA|NRO_COMPROB|RAZON|IMPORTE_TOTAL|TOTAL|COD_MONEDA|COTI_MONEDA|FECHA_EMISION|CONTENEDOR|BUQUE_NOMBRE|BUQUE_VIAJE|BUQUE_FECHA|CODE|CNT|IMP_UNIT|IMP_TOT|ISO\n";
                                            data.forEach(item => {
                                                response = response +
                                                    item.TERMINAL +
                                                    "|" +
                                                    ...
                                                    item.BUQUE_VIAJE +
                                                    "|" +
                                                    moment(item.BUQUE_FECHA).format("DD/MM/YYYY") +
                                                    "|" +
                                                    item.CODE +
                                                    "|" +
                                                    item.CNT +
