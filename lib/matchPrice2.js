            ...
            if (pool) {
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        var strSql = `SELECT TT.CODE CODE_TERMINAL, PRECIO, FECHA, MONEDA, T.*
                                    FROM TARIFARIO T
                                        LEFT JOIN TARIFARIO_TERMINAL TT ON T.ID = TT.TARIFARIO_ID AND TT.TERMINAL = :1
                                        LEFT JOIN (SELECT TP.*
                                                    FROM TARIFARIO_PRECIO TP
                                                    WHERE TP.FECHA = (SELECT MAX(FECHA) FROM TARIFARIO_PRECIO TP1 WHERE TP1.TARIFARIO_ID = TP.TARIFARIO_ID)) TPM
                                            ON TPM.TARIFARIO_ID = T.ID
                                    WHERE (T.TERMINAL = 'AGP' OR T.TERMINAL = :1)
                                    ORDER BY T.TERMINAL`;
                        connection.execute(strSql, [params.terminal], {})
                            .then(data => {
                                this.cn.doRelease(connection);
                                var rows = Enumerable.from(data.rows)
                                    .groupBy('x=>JSON.stringify({_id: x.ID, terminal: x.TERMINAL, code: x.CODE, description: x.DESCRIPCION, unit: x.UNIDAD, price: x.PRECIO, from: x.FECHA, currency: x.MONEDA})',
                                    null,
                                    (key, items) => {
                                        ...
                                        key.matches = [{match:[]}];
                                        prop.forEach(item => {
                                            key.matches[0].match.push(item.CODE_TERMINAL);
                                        });
                                        return key;
                                    })
                                    //.select(item => ({
