                        ...
                        var strSql = `SELECT TT.CODE CODE_TERMINAL, PRECIO, FECHA, MONEDA, T.*
                                    FROM TARIFARIO T
                                        ...
                                        LEFT JOIN (SELECT TP.*
                                                    ...
                                                    WHERE TP.ID = (SELECT MAX(TP1.ID)
                                                                      FROM TARIFARIO_PRECIO TP1
                                                                      WHERE TP1.TARIFARIO_ID = TP.TARIFARIO_ID)) TPM ON TPM.TARIFARIO_ID = T.ID
                                    WHERE (T.TERMINAL = 'AGP' OR T.TERMINAL = :1) ${onlyRates}
                                    ORDER BY T.TERMINAL`;
