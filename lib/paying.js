                            ...
                            strSql = `SELECT *
                                FROM (
                                  ...
                                  WHERE VHD.TERMINAL = :1 AND
                                        ${strWhere}
                                  GROUP BY VHD.ID, FECHA_EMISION, COD_TIPO_COMPROB, ABBREV, BUQUE_NOMBRE, RAZON, NRO_PTO_VENTA, NRO_COMPROB, COTI_MONEDA, VHD.CODE, USR, GRUPO, STATE, ${container} IMP_UNIT, PRECIO
                                )
                                ${strWhereGral}`;
                            connection.execute(strSql, [this.terminal], options, (err, invoices) => {
                                if (err) {
                                    asyncCallback(err);
                                } else {
                                    if (options.resultSet) {
                                        getResultSet(connection, invoices.resultSet, 500)
                                            .then(data => {
                                                console.log(data.length);
                                                asyncCallback(null, data);
                                                invoices.resultSet.close(err=> {
//                                                this.cn.doRelease(connection);
                                                });
                                        ...
                                        asyncCallback(null, invoices.rows);
                                    }
                                }
                            });
                        };
