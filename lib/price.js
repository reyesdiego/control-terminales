...
class PriceOracle {
                    ...
                    if (err) {
                        ...
                        connection.execute(strSql, param, {autoCommit: false}, (err, data) => {
                            if (err) {
                                ...
                                let task = (item, asyncCallbackPrice) => {
                                        ...
                                        var param = {
                                            tarifario_id: idNew,
                                            fecha: moment(item.from).format('YYYY-MM-DD'),
                                            price: item.price,
                                ...
                                async.eachSeries(params.topPrices, task, (err, data) => {
                                    if (err) {
    ...
    getPrice (id, callback) {
        ...
        if (pool) {
            ...
            pool.getConnection((err, connection) => {
                connection.execute(strSql, [id, self.terminal], {}, (err, data) => {
...
class PriceMongoDB {
        ...
        return new Promise((resolve, reject) => {
            ...
            _price.save((errSave, priceAdded) => {
                if (!errSave) {
                    ...
                    reject({status: "ERROR", data: errSave.message});
                }
            });
        });
    }
    ...
    update (params) {
        return new Promise((resolve, reject) => {
            this.model.findOne({_id: params._id}, (err, price2Upd) => {
                if (err) {
                    reject({
                        status: "ERROR",
                        message: err.message,
                    ...
                    if (price2Upd) {
                        ...
                        price2Upd.save((errSave, dataSaved) => {
                            if (!errSave) {
                                resolve({status: "OK", data: dataSaved});
                            } else {
                                reject({
                                    status: "ERROR",
