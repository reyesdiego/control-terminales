...
class PriceOracle {
                ...
                pool.getConnection((err, connection) => {
                    if (err) {
                        ...
                        var strSql = `INSERT INTO TARIFARIO (
                                    ...
                                    :descripcion,
                                    :unidad,
                                    :largo) RETURNING ID INTO :outputId`;
                        var param = {
                        ...
                        if (params.description !== undefined ||
                            ...
                            connection.execute(strSql, paramValues, {autoCommit: false}, (err, data) => {
                                if (err) {
                                    ...
                                    var strSql = `BEGIN DELETE FROM TARIFARIO_PRECIO WHERE TARIFARIO_ID = :1; DELETE FROM TARIFARIO_TERMINAL WHERE TARIFARIO_ID = :1  AND TERMINAL = :2; END;`;
                                    connection.execute(strSql, [params._id, params.matches[0].terminal], {autoCommit: false}, (err, data) => {
                                        if (err) {
                                            this.cn.doRelease(connection);
                                            reject({status: "ERROR", message: err.message, data: err});
                                        } else {
    ...
    getPrice (id) {
        return new Promise((resolve, reject) => {
            ...
            if (pool) {
                pool.getConnection((err, connection) => {
                    strSql = `SELECT T.ID, TERMINAL, CODE, DESCRIPCION, UNIDAD, RATE, FECHA, PRECIO, MONEDA, LARGO, NORMA
                        FROM TARIFARIO T
    ...
    getPrices (params) {
        return new Promise((resolve, reject) => {
            ...
            if (pool) {
                strSql = "SELECT TPP.PRECIO, TPP.MONEDA, T.CODE, T.DESCRIPCION, T.ID, T.TERMINAL, TP.FECHA, T.UNIDAD, T.RATE, T.LARGO, T.NORMA " +
                    "FROM TARIFARIO T " +
