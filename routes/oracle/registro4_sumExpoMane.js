...
module.exports = function (log, pool) {
    ...
    function getDistinct(req, res) {
        ...
        if (req.route.path === '/registro4_sumexpomane/contenedores') {
            distinct = 'CONTENEDOR';
        }
        ...
        pool.getConnection(function (err, connection) {
            ...
            if (err) {
                ...
                strSql = util.format("SELECT DISTINCT %s as D FROM REGISTRO4_SUMEXPOMANE WHERE %s is not null ORDER BY %s", distinct, distinct, distinct);
    ...
    router.get('/registro4_sumexpomane/contenedores', getDistinct);
